...
main(int argc, char *argv[])
{
	...
	gnome_score_init("gtetris");

	gnome_init("gtetris", TETRIS_VERSION, argc, argv);

	GnomeClient *client= gnome_master_client();
	
	char *pixname, *fullpixname;
	pixname = g_copy_strings( "gtetris/", "5blocks.png", NULL);
	fullpixname = gnome_unconditional_pixmap_file(pixname);
	g_free(pixname);

	if (!g_file_exists(fullpixname)) 
	{
		printf(_("Could not find the \'%s\' theme for gtetris\n"), fullpixname);
		exit(1);
	}
